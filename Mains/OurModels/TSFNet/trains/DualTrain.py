...
def train_model(train_dataloader, test_dataloader, model, batch_size, num_epoch, loss_fn,
    ...
    for epoch in range(1, num_epoch + 1):
        model.train()
        for (X, y) in enumerate(zip(train_dataloader['v1'], train_dataloader['v2'])):
            input_data = torch.cat((y[0][0], y[1][0]), dim=0)
        ...
        model.eval()
        for (X, y) in enumerate(zip(test_dataloader['v1'], test_dataloader['v2'])):
            input_data = torch.cat((y[0][0], y[1][0]), dim=0)
            input_data = input_data.cuda()
            output_data = torch.as_tensor(y[0][1])
            output_data = output_data.cuda()
...
if __name__ == "__main__":
    ...
    model = DualModels.RDNet().to(device)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    num_epochs = 200
    # loss_fn = nn.CrossEntropyLoss()
    loss_fn = DualModels.ContrastiveLoss()
    path_name = './log1/rdnet/'
    idx = 3
