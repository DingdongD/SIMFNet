...
train_dataloader['v1'] = DataLoader(dataset=train_rt_dataset, batch_size=batch_size, shuffle=False, pin_memory=True,
                                    sampler=mySampler, drop_last=True)
...
def train_model(train_dataloader, test_dataloader, model, batch_size, num_epochs, loss_fn,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch, (X, y) in enumerate(train_dataloader):
            ...
            train_acc.append((output.argmax(1) == y).type(torch.float).sum().item() / batch_size)
        ...
        for X, y in test_dataloader:
            ...
            test_acc.append((output.argmax(1) == y).type(torch.float).sum().item() / batch_size)
...
if __name__ == "__main__":
    ...
    model = SoloModels.BiLSTM().to(device)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    num_epochs = 300
    loss_fn = nn.CrossEntropyLoss()
    path_name = 'log/lstm1/'
    idx = 10
    model, avg_train_loss, avg_test_loss, avg_train_acc, avg_test_acc, minposs = train_model(train_dataloader['v1'],
                                                                                             test_dataloader['v1'],
                                                                                             model,
